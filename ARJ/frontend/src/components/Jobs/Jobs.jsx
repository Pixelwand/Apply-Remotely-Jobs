  ...
  return (
    <>
      <div className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white font-sans font-bold p-10 pt-2 text-2xl">
        <div className="mb-5">
          <a href="/" className="text-base leading-9">
            Apply Remotely Jobs
          </a>
        </div>
        List of all Remote Jobs
      </div>
      <div>
        {jobData.map((job) => (
          <div
            key={job.id}
            className="bg-gradient-to-r from-green-400 to-blue-500 hover:from-pink-500 hover:to-yellow-500 my-5 p-3"
          >
            <div className="flex flex-row gap-24">
              <div className="basis-1/2">
                <div
                  key={job.id}
                  className="font-bold font-sans text-2xl leading-10"
                >
                  {job.position}
                </div>
                <div
                  key={job.id}
                  className="font-semibold text-xl leadin-16 font-sans"
                >
                  {job.companyName}
                </div>
            ...
            <div className="flex flex-row gap-28 mt-5">
              ...
              <div
                className="bg-white px-6 py-2 font-semibold rounded-full"
                key={job.id}
              >
